...
def get_embedding_model():
    ...
    print(f"正在实例化Embedding模型: {active_model_id} (类: {EmbeddingClass.__name__})")
...
if __name__ == '__main__':
    try:
        print("--- 测试Embedding模型提供程序 ---")
        embedding_model = get_embedding_model()
        print(f"成功获取实例: {type(embedding_model)}")
        
        # 测试向量化
        print("\n正在测试文本向量化...")
        test_vector = embedding_model.embed_query("这是一个测试句子。")
        print(f"向量化成功！维度: {len(test_vector)}")
